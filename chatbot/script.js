...
const HF_TOKEN = "" // Debes configurar tu token en la consola de GitHub Secrets
const MODEL_ID = "HuggingFaceH4/zephyr-7b-beta" // Modelo a utilizar

// Intenta cargar el token desde una variable de entorno si está disponible
// (esto funcionará cuando configures los secretos en GitHub)
...
async function sendMessageToModel(message) {
  try {
    ...
    if (!HF_TOKEN) {
      return "⚠️ Error: No se ha configurado el token de Hugging Face. Por favor, configura el token en las variables de entorno o directamente en el código."
    }
